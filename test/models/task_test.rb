...
class TaskTest < ActiveSupport::TestCase
  ...
  def test_incremental_slug_generation_for_tasks_with_duplicate_titles
  ...
  def test_existing_slug_prefixed_in_new_task_title_doesnt_break_slug_generation
    existing_task = Task.create!(title: "buy milk and apple", assigned_user: @user, task_owner: @user)
    assert_equal "buy-milk-and-apple", existing_task.slug

    new_task = Task.create!(title: "buy milk", assigned_user: @user, task_owner: @user)
    assert_equal "buy-milk", new_task.slug
  end

  def test_having_numbered_slug_substring_in_title_doesnt_affect_slug_generation
    existing_task = Task.create!(title: "buy 2 apples", assigned_user: @user, task_owner: @user)
    assert_equal "buy-2-apples", existing_task.slug

    new_task = Task.create!(title: "buy", assigned_user: @user, task_owner: @user)
    assert_equal "buy", new_task.slug
  end
